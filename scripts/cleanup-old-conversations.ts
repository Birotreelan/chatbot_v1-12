/**
 * Script para eliminar conversaciones mayores a 30 días
 * Ejecutar con: npx tsx scripts/cleanup-old-conversations.ts
 */
...
async function getLastMessageDate(redis: Redis, conversationKey: string): Promise<Date | null> {
  try {
    ...
    console.error(`Error leyendo último mensaje de ${conversationKey}:`, error)
    return null
  }
}

async function cleanupOldConversations(): Promise<CleanupStats> {
  ...
  const contactKeysToDelete: string[] = []
  const phoneNumbersToRemoveFromSets: Map<string, string[]> = new Map()
  ...
  for (const conversationKey of conversationKeys) {
    ...
    try {
      ...
      // Si la última actividad es anterior a la fecha de corte, marcar para eliminar
      if (lastMessageDate < CUTOFF_DATE) {
        keysToDelete.push(conversationKey)
        
        // Contar mensajes que se eliminarán
        const messageCount = await redis.llen(conversationKey)
        stats.messagesDeleted += messageCount

        // Extraer configId y phoneNumber
        const parts = conversationKey.replace(CONVERSATION_PREFIX, "").split(":")
        if (parts.length >= 2) {
          const configId = parts[0]
      ...
      console.error(`Error procesando ${conversationKey}:`, error)
      stats.errors++
    }
  }
  ...
  // Eliminar conversaciones y claves de pausa
  ...
  for (let i = 0; i < allKeysToDelete.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      console.log(`  Eliminadas ${Math.min(i + BATCH_SIZE, allKeysToDelete.length)}/${allKeysToDelete.length} claves`)
    } catch (error) {
  ...
  for (const [configId, phoneNumbers] of phoneNumbersToRemoveFromSets) {
    ...
    try {
      if (phoneNumbers.length > 0) {
        ...
        console.log(`  Removidos ${phoneNumbers.length} contactos del set ${configId}`)
      }
    } catch (error) {
      console.error(`Error removiendo del set ${setKey}:`, error)
      stats.errors++
    }
  }
