...
async function getLastMessageDate(redis, conversationKey) {
  try {
    ...
    console.error(`[v0] Error leyendo último mensaje de ${conversationKey}:`, error.message);
    return null;
  }
}
...
async function cleanupOldConversations() {
  ...
  console.log("[v0] =".repeat(60));
  console.log("[v0] LIMPIEZA DE CONVERSACIONES ANTIGUAS");
  console.log("[v0] =".repeat(60));
  console.log(`[v0] Fecha de corte: ${CUTOFF_DATE.toISOString()}`);
  console.log(`[v0] Se eliminarán conversaciones anteriores a ${DAYS_TO_KEEP} días`);
  console.log("[v0] ");
  ...
  try {
    ...
    console.log("[v0] Conectando a Redis...");
    
    // Test de conexión
    const testKey = `cleanup-test-${Date.now()}`;
    await redis.set(testKey, "test");
    await redis.del(testKey);
    console.log("[v0] Conexión a Redis exitosa");

    // 1. Escanear todas las conversaciones
    console.log("[v0] Escaneando conversaciones...");
    const conversationKeys = await scanRedisKeys(redis, `${CONVERSATION_PREFIX}*`);
    console.log(`[v0] Encontradas ${conversationKeys.length} conversaciones`);
    ...
    console.log("[v0] Analizando fechas de conversaciones...");
    let processed = 0;

    for (const conversationKey of conversationKeys) {
      processed++;
      if (processed % 100 === 0) {
        console.log(`[v0]   Procesadas ${processed}/${conversationKeys.length}...`);
      }
      ...
      try {
        ...
        console.error(`[v0] Error procesando ${conversationKey}:`, error.message);
        stats.errors++;
      }
    }
    ...
    console.log("[v0] ");
    console.log("[v0] =".repeat(60));
    console.log("[v0] RESUMEN DE LIMPIEZA");
    console.log("[v0] =".repeat(60));
    console.log(`[v0] Conversaciones a eliminar: ${keysToDelete.length}`);
    console.log(`[v0] Contactos a eliminar: ${contactKeysToDelete.length}`);
    console.log(`[v0] Mensajes totales a eliminar: ${stats.messagesDeleted}`);
    console.log("[v0] ");

    if (keysToDelete.length === 0) {
      console.log("[v0] No hay conversaciones antiguas para eliminar.");
      return stats;
    }

    // 3. Eliminar las claves en lotes
    console.log("[v0] Eliminando claves...");
    ...
    for (let i = 0; i < allKeysToDelete.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        console.log(
          `[v0]   Eliminadas ${Math.min(i + BATCH_SIZE, allKeysToDelete.length)}/${allKeysToDelete.length} claves`
        );
      } catch (error) {
        console.error(`[v0] Error eliminando lote:`, error.message);
        stats.errors++;
      }
    }
    ...
    console.log("[v0] Limpiando sets de contactos...");
    for (const [configId, phoneNumbers] of phoneNumbersToRemoveFromSets) {
      ...
      try {
        if (phoneNumbers.length > 0) {
          ...
          console.log(
            `[v0]   Removidos ${phoneNumbers.length} contactos del set ${configId}`
          );
        }
      } catch (error) {
        console.error(`[v0] Error removiendo del set ${setKey}:`, error.message);
        stats.errors++;
      }
    }
    ...
    console.log("[v0] =".repeat(60));
    console.log(`[v0] Conversaciones eliminadas: ${stats.conversationsDeleted}`);
    console.log(`[v0] Contactos eliminados: ${stats.contactsDeleted}`);
    console.log(`[v0] Mensajes eliminados: ${stats.messagesDeleted}`);
    console.log(`[v0] Errores: ${stats.errors}`);
    console.log("[v0] ");
