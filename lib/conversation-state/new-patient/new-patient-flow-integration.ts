...
export interface NewPatientResult {
  ...
  message?: string
  /**
   * Nota corta y aislada, redactada específicamente para el widget
   * ("Gracias, te agendaremos como paciente nuevo.") — sólo se completa en
   * la inicialización del flujo (primer paso tras el DNI). Separado de
   * `message` por el mismo motivo que en existing-patient-flow-integration.ts:
   * el widget de formulario ya tiene su propio texto fijo para el paso de
   * obra social, no necesita repetir la instrucción completa pensada para
   * WhatsApp.
   */
  greeting?: string
  action?: string
 ...
 * para construir una respuesta estructurada (opciones de obra social/sede/
 * profesional/especialidad, turnos con fecha y hora) en vez de sólo texto
 * plano. No modifica nada, es un espejo de `getFlowState`.
 */
export async function getNewPatientFlowSnapshot(phone: string): Promise<NewPatientFlowState | null> {
...
export async function initializeNewPatientFlow(
  ...
  const greeting = state.esFamiliar
    ? 'Gracias, agendaremos al familiar como paciente nuevo.'
    : 'Gracias, te agendaremos como paciente nuevo.'
      ...
      if (state.turnosOpciones && state.turnosOpciones.length > 0) {
        ...
        return {
          ...
          message: withBackOption(
            buildTurnosWindowMessage(window.turnos, state.turnosOpciones.length, window.hasMore, nombreCompleto || undefined, state.sedeNombre, state.profesionalNombre, true),
...
async function handleTurnoPhase(
  ...
  // Si hay un turno pendiente de confirmar (resuelto por texto/hora/NLU), interpretamos
  // la respuesta: Sí → proceder; No → re-mostrar lista; número → nueva selección.
  if (state.turnoPendingConfirm) {
    ...
    if (isAffirmative(userMessage) || /^s[ií]\b/.test(norm) || norm === 'confirmo') {
      ...
      await saveFlowState(phone, state)
      const nombre = `${state.nombre || ''} ${state.apellido || ''}`.trim()
      return {
        handled: true,
        message: withBackOption(
          buildTurnosWindowMessage(w.turnos, state.turnosOpciones.length, w.hasMore, nombre || undefined, state.sedeNombre, state.profesionalNombre, true),
          'awaiting_turno_selection',
          'new',
        ),
