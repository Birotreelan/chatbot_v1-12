...
export async function processIndividualMessage(
  ...
  try {
    console.log(`[WHATSAPP-PROCESSOR] Processing individual message for ${userPhoneNumber}`)

    // Convert the parameters to match processWhatsAppMessage signature
    ...
    console.log(`[WHATSAPP-PROCESSOR] Individual message processed successfully: ${response.length} characters`)
  } catch (error) {
    console.error(`[WHATSAPP-PROCESSOR] Error processing individual message:`, error)
    throw error
  }
}

export async function handleMessage(webhookData: any): Promise<void> {
  try {
    console.log(`[WHATSAPP-PROCESSOR] Handling webhook message`)

    if (!webhookData.messages || !webhookData.messages[0]) {
      throw new Error("No message data found in webhook")
    }
    ...
    // Get WhatsApp config based on phone number ID
    const { getWhatsAppConfigByPhoneId } = await import("@/lib/db")
    ...
    await sendWhatsAppMessage(phoneNumber, response, config)

    console.log(`[WHATSAPP-PROCESSOR] Webhook message handled successfully`)
  } catch (error) {
    console.error(`[WHATSAPP-PROCESSOR] Error handling webhook message:`, error)
    throw error
  }
}
